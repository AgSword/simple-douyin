...
func (s *RegisterService) Run(req *user.UserRegisterRequest) (resp *user.UserRegisterResponse, err error) {
	...
	userIDb := mysql.User{Name: req.Username, FollowCount: 0, FollowerCount: 0}
	err = mysql.CreateUser(s.ctx, &userIDb)
	if err != nil {
		return nil, err
	}
	// 构造密码
	pw := md5.MD5(strconv.FormatInt(userIDb.ID, 10) + req.Password + strconv.FormatInt(userIDb.ID, 10))
	userIDb.Password = pw
	// 将密码保存到数据库
	...
	loginRequest := user.UserLoginRequest{Username: userIDb.Name, Password: userIDb.Password}
